...
place_model = ns.model(
    "Place",
    {
        ...
        "max_guest": fields.Integer(
            required=True, min=1, description="Maximum spirits allowed", example=4
        ),
        "price_by_night": fields.Float(
            required=True, min=0.0, description="Price per haunted night", example=100.0
        ),
...
place_review_model = ns.model(
    "PlaceReview",
    {
        ...
        "rating": fields.Integer(
            required=True, min=1, max=5, description="Spooky rating"
        ),
    },
)
...
class PlaceList(Resource):
    ...
    @ns.doc(
        "list_places",
        responses={200: "Success", 400: "Invalid parameters", 404: "No places found"},
    )
        ...
        try:
            ...
            if all(k in request.args for k in ["latitude", "longitude"]):
                ...
                return Place.get_by_location(
                    latitude, longitude, float(request.args.get("radius", 10.0))
                )
...
class PlaceResource(Resource):
    ...
    @ns.doc(
        "update_place",
        responses={200: "Success", 400: "Invalid parameters", 404: "Place not found"},
    )
    @ns.expect(place_model)
    @ns.marshal_with(place_model)

...
def test_user_update():
    ...
    user.update(
        {"username": "super_ghost", "first_name": "Super", "last_name": "Ghost"}
    )
    ...
    user.update({"address": "666 Spooky Lane", "phone": "+1-666-BOO"})
...
def test_user_delete_with_reviews():
    ...
    # Now create review
    review = Review(user_id=user.id, place_id=place.id, text="Spooky place!", rating=5)
    review.save()
...
def test_user_get_by_combined_filters(repository):
    ...
    found = User.get_by_attr(city="Ghostville", is_admin=True, is_active=True)
    assert found is not None

    found_multiple = User.get_by_attr(multiple=True, city="Ghostville", is_admin=True)
    assert len(found_multiple) == 2

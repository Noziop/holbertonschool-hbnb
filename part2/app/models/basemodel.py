...
class BaseModel:
    ...
    def save(self) -> T:
        ...
        try:
            self.logger.debug(
                f"Attempting to save {self.__class__.__name__} with ID: {self.id}"
            )
            self.repository.add(self)
            self.logger.info(
                f"Successfully saved {self.__class__.__name__} with ID: {self.id}"
            )
            return self
    ...
    def update(self, data: dict) -> T:
        ...
        try:
            self.logger.debug(
                f"Attempting to update {self.__class__.__name__} with ID: {self.id}"
            )
    ...
    def hard_delete(self) -> bool:
        ...
        try:
            self.logger.debug(
                f"Attempting hard delete of {self.__class__.__name__} with ID: {self.id}"
            )
            ...
            self.logger.error(
                f"Hard delete failed for {self.__class__.__name__}: {str(e)}"
            )
            raise
